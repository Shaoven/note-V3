...
class TestUpdateRemindNote(unittest.TestCase):
    ...
    def testCase01_userB_update_userA_RemindNote(self):
        """用户A更新用户B的日历便签内容"""
        info('STEP:新建日历便签主体')
    ...
    def testCase02_update_recycle_RemindNote(self):
        """用户有1条日历便签在回收站，更新这个便签"""
        info('STEP:新建日历便签主体')
        note_id = str(int(time.time() * 1000)) + '_test_noteId'
        remind_time = int(time.time() * 1000)
        get_note_info_body = {
    ...
    def testCase03_update_delete_note(self):
        """用户有1条日历便签在回收站中被清空，更新这个便签"""
        info('STEP:新建便签主体')
        note_id = str(int(time.time() * 1000)) + '_test_noteId'
        remind_time = int(time.time() * 1000)
        get_note_info_body = {
    ...
    def testCase04_update_recover_note(self):
        """更新在回收站中恢复的日历便签内容"""
        info('STEP:新建日历便签主体')
        note_id = str(int(time.time() * 1000)) + '_test_noteId'
        remind_time = int(time.time() * 1000)
