...
class TestGetRecycleNote(unittest.TestCase):
    ...
    url = host + path
    apiRe = ApiRe()
    special = apiConfig['GetRecycleNote']['special']
    ...
    def testCase01_sid_not_exist(self):
        """查看回收站下便签 sid不存在"""
        sid = '123asd'
        res = self.apiRe.note_get(self.url, sid)
        self.assertEqual(401, res.status_code, msg='状态码异常')  # 先描述期望值，再描述结果

    def testCase02_user_id_is_too_long(self):
        """查看回收站下便签 user_id的长度=1000"""
        user_id = '5' * 1000
        start_index = 0
        rows = 1
        path = f"/v3/notesvr/user/{user_id}/invalid/startindex/{start_index}/rows/{rows}/notes"
        url = self.host + path
    ...
    def testCase11_startIndex_input_min_number(self):
        """ 查看回收站下便签 startIndex输入最小值：-2147483649 """
        user_id = '505807555'
    ...
    def testCase12_startIndex_input_max_number(self):
        """ 查看回收站下便签 startIndex输入最大值：2147483648 """
        user_id = '505807555'
    ...
    def testCase14_startIndex_input_string_number(self):
        """ 查看回收站下便签 startIndex输入字符串形式的数值：“1” """
        user_id = '505807555'
        start_index = '1'
        rows = 1
        path = f"/v3/notesvr/user/{user_id}/invalid/startindex/{start_index}/rows/{rows}/notes"
